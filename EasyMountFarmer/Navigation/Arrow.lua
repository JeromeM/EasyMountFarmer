-- Arrow.lua — our own on-screen "point me there" arrow, replacing TomTom's crazy
-- arrow. It renders ONLY direction + distance to the destination that Waypoint
-- handed us; routing (which portal / item / hop to take) stays with FarstriderLib
-- (Nav.lua). No external library: player and target world positions both come
-- from the native C_Map API, and the heading uses GetPlayerFacing().
--
...
---@param tex table  the arrow Texture
---@param angle number  radians, 0 = point up/away, increasing CCW
...
-- last computed values, exposed for the /emf nav calibration dump
...
---@return number r, number g, number b  each 0-1
...
--- Distance (yards) -> colour, hue interpolated red→orange→yellow→green as you get
--- closer. Log scale so the useful WoW distances spread out nicely (green very
--- close, yellow ~a few hundred yd, orange ~mid, red far). Clean smooth transitions.
---@param d number  distance in yards
...
---@return number? wx  world X (north axis, yards)
---@return number? wy  world Y (west axis, yards)
...
---@return table  the arrow Button frame
function Arrow.Ensure()
  if Arrow.frame then return Arrow.frame end
  local f = CreateFrame("Button", "EasyMountFarmerArrow", UIParent)
  Arrow.frame = f
  ...
  -- text stack: final target (amber) / router instruction (white) / distance.
  -- Fonts are one notch smaller than default; the Text-size slider scales the lot.
...
---@param owner table  the frame to anchor the menu to
function Arrow.OpenMenu(owner)
  if not (MenuUtil and MenuUtil.CreateContextMenu) then return end
  MenuUtil.CreateContextMenu(owner, function(_, root)
    root:CreateTitle(L["EasyMountFarmer"])
...
function Arrow.Show(map, x, y, title)
  ...
  Arrow.Ensure()
  -- Top line = the current FARM target (stable across hops), amber like the main
  -- window. Second line = the router's own step instruction (title), in white.
  local nameLine
  if ns.UI and ns.UI.ArrowText then
    nameLine = ns.UI.ArrowText(ns.Progress and ns.Progress.Current())
  end
  Arrow.frame.name:SetText(nameLine or "")
  -- Don't echo the instruction when it's just the target name again (entrance fallback).
